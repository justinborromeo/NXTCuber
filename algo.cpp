...
// We will store the inputs of faces as Up, Down, Left, Right, Front, and Back.
// This makes the program independent of what color the faces actually are, so that
// UF UR UB UL DF DR DB DL FR FL BR BL UFR URB UBL ULF DRF DFL DLB DBR
// is the correctly solved format. If, for example, the input is
// Correct: UF UR UB UL DF DR DB DL FR FL BR BL UFR URB UBL ULF DRF DFL DLB DBR
// Input:   RU LF UB DR DL BL UL FU BD RF BR FD LDF LBD FUL RFD UFR RDB UBL RBU
// That means the cubelet in the Up-Front position is the Right-Up cubelet, and so on.
// If the Up face is white an the Front face is red, then this means that the blue-white
// cubelet is position between the white and red middle pieces such that the blue color
// is on the white side.

// We will use char arrays because initialization is far more concise.
// The list of faces in order of importance such that a cubelet RU is in correct
// orientation but UR is in incorrect orientation.
char *FACES = "RLFBUD";
// We will reorder the input cubelets slightly to a form more convenient for
// Thistlethwaite's algorithm
char *REORDER = "AECGBFDHIJKLMSNTROQP";
// 
char *MAP = "TdXhQaRbEFIJUZfijeYV";
...
int main(int argc, char **argv) {
    ...
    for (; i < 20; i++) {
        ...
        for (f = 0; f < 20; f++) {
            if (pc == MAP[f] - 64) { break; }
        }
        // store piece
        pos[REORDER[i] - CHAROFFSET] = f;
        ori[REORDER[i] - CHAROFFSET] = mor % val[i];
    }
