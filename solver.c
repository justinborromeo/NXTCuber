...
char seq[256][256], output[256] = "", *s, t[256];
...
HEURISTIC(hAll, EDGE(0, 24)
        CORNER
        TEST(sum - 16 ? sum - 13 ? sum - 8 ? 3 : 1 : 2 : 2))

HEURISTIC(hBackCross, EDGE(0, 8))

HEURISTIC(hLayer23, EDGE(8, 24)
        TEST(sum > 5 ? 5 : sum > 2 ? sum - 2 : 1))

HEURISTIC(hCorners, CORNER
        TEST(sum > 6 ? 5 : sum - 1))
...
int main(int argc, char **argv) {
    FOR(i, 120) {
        rotColor[i] =
                "^om`FUNW]nl_VMXEpka\\hkneIOMKofilNLJPmpgjdigZBSJQchfYATIRbje[aljcHXPTkib`OSGWhd_m[fp]RLVD\\ZeoCQKUYgn^ad[^HBDFcZ]`_bY\\EGAC"[i] -
                65;
    }
